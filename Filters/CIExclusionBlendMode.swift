...
class CIExclusionBlendMode: BaseFilter {
    ...
    private func setupDescription() {
        self.descView.text = """
        滤镜：CIExclusionBlendMode
        系统：iOS6.0
        简介：Produces an effect similar to that produced by the CIDifferenceBlendMode filter but with lower contrast.
        详情：Source image sample values that are black do not produce a change; white inverts the background color values. The formula used to create this filter is described in the PDF specification, which is available online from the Adobe Developer Center. See PDF Reference and Adobe Extensions to the PDF Specification.
        """
    }
