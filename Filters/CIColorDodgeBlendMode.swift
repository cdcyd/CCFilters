...
class CIColorDodgeBlendMode: BaseFilter {
    ...
    private func setupDescription() {
        self.descView.text = """
        滤镜：CIColorDodgeBlendMode
        系统：iOS6.0
        简介：Brightens the background image samples to reflect the source image samples.
        详情：Source image sample values that specify black do not produce a change. The formula used to create this filter is described in the PDF specification, which is available online from the Adobe Developer Center. See PDF Reference and Adobe Extensions to the PDF Specification.
        """
    }
